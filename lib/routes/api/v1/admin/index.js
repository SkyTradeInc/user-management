...
const isSubscriber = (request, response, next) => {
  if(request.decoded.role !== "Member" || request.decoded.role !== "Subscriber") return errorResponse(response, 'Insufficient privileges');
  next()
};
